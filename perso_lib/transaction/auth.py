...
def get_issuer_pub_key(ca_pub_key,ca_exp,tag90,tag92,tag9F32,tag5A,tag5F24):
    ...
    if not recovery_data:
        Log.info('can not require recovery data through public cert tag90')
        return issuer_pub_key
    Log.info('issuer recovery data: %s',recovery_data)
    if recovery_data[0:4] != '6A02' or recovery_data[-2:] != 'BC':
        Log.info('recovery data format incorrect, it should start with 6A02 and end with BC')
        return issuer_pub_key
    pan = recovery_data[4:12].replace('F','')
    if len(pan) < 3 or not tag5A.startswith(pan):
        Log.info('pan required from recovery data is not accord with tag5A')
        return issuer_pub_key
    expiry_date = int(recovery_data[14:16] + recovery_data[12:14])
    now = int(time.strftime('%C%m'))
    if expiry_date < now:
        Log.info('issuer cert has been overdue')
        # return issuer_pub_key
    if expiry_date < int(tag5F24[0:4]):
        Log.info('issuer cert overdue earlier than application expiry date')
        return issuer_pub_key
    if recovery_data[22:26] != '0101':
        Log.info('unknown hash algo flag')
        return issuer_pub_key
    ...
    if hash_cert != hash_output:
        Log.info('hash compared failed whereby gen issuer recovery data failed ')
        return issuer_pub_key
...
def get_icc_pub_key(issuer_pub_key,tag9F32,tag9F46,tag9F48,tag9F47,sig_data,tag5A,tag5F24):
    ...
    now = int(time.strftime('%C%m'))
    if expiry_date < now:
        Log.info('icc cert has been overdue')
        # return icc_pub_key
    if expiry_date < int(tag5F24[0:4]):
    ...
    hash_output = algorithm.gen_hash(hash_input)
    if hash_cert != hash_output:
        Log.info('hash compared failed whereby gen icc recovery data failed ')
        return icc_pub_key
...
def validate_9F4B(icc_pub_key,icc_exp,ddol,tag9F4B):
    ...
    if recovery_data[0:4] not in ('6A05','6A95') or recovery_data[-2:] != 'BC':
        Log.info('recovery data format incorrect, it should start with 6A05/6A95 and end with BC')
        return False
    hash_input = recovery_data[2:-42] + ddol
    hash_9F4B = recovery_data[-42:-2]
    hash_output = algorithm.gen_hash(hash_input)
    if hash_9F4B != hash_output:
        Log.info('hash compared failed whereby tag9F4B is not correct ')
        return False
